...
export default function EReportView({ team, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#FAF2EC]/50 backdrop-blur-md overflow-y-auto">
      <div className="w-full max-w-4xl bg-white border border-[#EADBD0] rounded-3xl shadow-2xl overflow-hidden my-8 flex flex-col max-h-[90vh]">
        
        {/* Action Header bar */}
        <div className="flex items-center justify-between p-4 border-b border-[#EADBD0] bg-[#FAF2EC] no-print">
          <div className="flex items-center gap-2">
            <FileText className="w-5 h-5 text-[#FF5F38]" />
            <h2 className="text-sm font-bold text-[#111827]">Auto-Generated Structured E-Report (6th Semester CIA)</h2>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handlePrint} className="px-5 py-2.5 bg-[#FF5F38] hover:bg-[#E54D26] text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer py-1.5 text-xs">
        ...
        <div className="p-8 bg-white text-[#111827] overflow-y-auto space-y-6 print:p-4">
          
          {/* Cover Header */}
          <div className="text-center border-b border-[#EADBD0] pb-6">
            <div className="text-xs uppercase font-bold tracking-widest text-[#FF5F38] mb-1">
            ...
            <div className="inline-block px-3 py-1 rounded-full bg-[#FF5F38]/10 text-blue-800 font-mono text-xs border border-blue-200">
              CIA Evaluation Subject Code: BCA-601P (50 Marks Total / 2 Credits)
            </div>
          </div>

          {/* Section 1: Project Details */}
          <div className="bg-white border border-[#EADBD0] shadow-sm p-5 rounded-2xl border-[#EADBD0] bg-[#FAF2EC]/50">
            <h2 className="text-xs font-bold uppercase tracking-wider text-[#FF5F38] mb-3 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div>
                ...
                <p className="text-[#111827] font-bold text-sm">{team.name} ({team.id})</p>
              </div>
              <div>
                <span className="text-slate-500 font-medium">Assigned Mentor:</span>
                <p className="text-[#111827] font-bold text-sm">{team.mentorName || 'Unassigned'}</p>
              </div>
              <div>
                <span className="text-slate-500 font-medium">Selected Domain:</span>
                <p className="text-[#FF5F38] font-bold">{team.domain || 'N/A'}</p>
              </div>
              <div>
          ...
          <div className="bg-white border border-[#EADBD0] shadow-sm p-5 rounded-2xl border-[#EADBD0]">
          ...
          <div className="bg-white border border-[#EADBD0] shadow-sm p-5 rounded-2xl border-[#EADBD0]">
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <p className="text-slate-800 mt-1 p-3 rounded-xl bg-[#FAF2EC] border border-[#EADBD0]">
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {team.researchPapers.map((paper, idx) => (
                    <tr key={idx}>
                      <td className="py-2.5 px-2 font-mono font-bold text-[#FF5F38]">{idx + 1}</td>
                      <td className="py-2.5 px-3 font-semibold text-[#111827]">{paper.title}</td>
          ...
          <div className="p-5 rounded-2xl bg-[#FF5F38]/10/70 border border-blue-200">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-center text-[11px] pt-2">
              <div className="p-2 rounded-xl bg-white border border-blue-100 shadow-sm">
                ...
                <div className="font-bold text-[#111827] font-mono mt-1">{team.marks?.cia?.teamFormation || 5} / 5</div>
              </div>
              <div className="p-2 rounded-xl bg-white border border-blue-100 shadow-sm">
                <div className="text-slate-500">Mentor Select</div>
                <div className="font-bold text-[#111827] font-mono mt-1">{team.marks?.cia?.mentorSelection || 5} / 5</div>
              </div>
              <div className="p-2 rounded-xl bg-white border border-blue-100 shadow-sm">
                <div className="text-slate-500">Domain ID</div>
                <div className="font-bold text-[#111827] font-mono mt-1">{team.marks?.cia?.domainSelection || 5} / 5</div>
              </div>
              <div className="p-2 rounded-xl bg-white border border-blue-100 shadow-sm">
                <div className="text-slate-500">Problem ID</div>
                <div className="font-bold text-[#111827] font-mono mt-1">{team.marks?.cia?.problemIdentification || 5} / 5</div>
              </div>
              <div className="p-2 rounded-xl bg-white border border-blue-100 shadow-sm">
                <div className="text-slate-500">Literature Rev</div>
                <div className="font-bold text-[#111827] font-mono mt-1">{team.marks?.cia?.researchReview || 5} / 5</div>
              </div>
            </div>
          </div>

          {/* Footer Signatures */}
          <div className="pt-10 grid grid-cols-3 gap-4 text-center text-xs border-t border-[#EADBD0]">
            <div>
              <div className="h-8 border-b border-dashed border-[#EADBD0] mb-1"></div>
              <p className="font-bold text-[#111827]">Team Leader Signature</p>
              <p className="text-[10px] text-slate-500">Date: {new Date().toLocaleDateString()}</p>
            </div>
            <div>
              <div className="h-8 border-b border-dashed border-[#EADBD0] mb-1"></div>
              <p className="font-bold text-[#111827]">Project Mentor Signature</p>
              <p className="text-[10px] text-slate-500">{team.mentorName || 'Faculty Supervisor'}</p>
            </div>
            <div>
              <div className="h-8 border-b border-dashed border-[#EADBD0] mb-1"></div>
              <p className="font-bold text-[#111827]">Project Coordinator Signature</p>
