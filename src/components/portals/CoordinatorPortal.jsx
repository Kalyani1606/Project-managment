...
export default function CoordinatorPortal() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-[#EADBD0] pb-3 overflow-x-auto">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setActiveTab('dashboard')}
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
              activeTab === 'dashboard'
                ? 'bg-[#FAF2EC] text-white shadow-md'
        ...
        <div className="hidden sm:flex items-center gap-2 text-xs font-mono text-slate-500">
          <span className="text-[#FF5F38] font-bold">● {profile.fullName}</span>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
            <div className="bg-white border border-[#EADBD0] shadow-sm p-4 rounded-2xl border border-[#EADBD0] bg-white flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xl font-extrabold text-[#111827] font-mono">240</div>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#EADBD0] shadow-sm p-4 rounded-2xl border border-[#EADBD0] bg-white flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xl font-extrabold text-[#111827] font-mono">{data.mentors.length}</div>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#EADBD0] shadow-sm p-4 rounded-2xl border border-[#EADBD0] bg-white flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xl font-extrabold text-[#111827] font-mono">{data.reviews.length}</div>
              </div>
            </div>
          </div>

          <div className="p-6 rounded-3xl border border-[#EADBD0] bg-white shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
        ...
        <div className="p-6 rounded-3xl border border-[#EADBD0] bg-white shadow-md space-y-4">
          <div className="flex items-center justify-between border-b border-[#EADBD0] pb-4">
            <h2 className="text-sm font-bold text-[#111827] flex items-center gap-2">
              <Users className="w-4 h-4 text-[#FF5F38]" /> Master Team Management Panel
            </h2>
          ...
          <div className="space-y-4">
            {data.teams.map(team => (
              <div key={team.id} className="p-4 rounded-2xl bg-[#FAF2EC] border border-[#EADBD0] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono text-[#FF5F38] font-bold">{team.id}</span>
      ...
      {activeTab === 'mentors' && (
        <div className="p-6 rounded-3xl border border-[#EADBD0] bg-white shadow-md space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.mentors.map(m => (
              <div key={m.id} className="p-4 rounded-2xl bg-[#FAF2EC] border border-[#EADBD0] space-y-3">
      ...
      {activeTab === 'reviews' && (
        <div className="space-y-6">
          <div className="p-6 rounded-3xl border border-[#EADBD0] bg-white shadow-md max-w-3xl mx-auto space-y-4">
            <h2 className="text-sm font-bold text-[#111827] flex items-center gap-2">
              <Calendar className="w-4 h-4 text-[#FF5F38]" /> Schedule New Semester Review
            </h2>
          ...
          <div className="p-6 rounded-3xl border border-[#EADBD0] bg-white shadow-md space-y-4">
            <h3 className="text-sm font-bold text-[#111827]">Scheduled Review Events</h3>
            <div className="space-y-3">
              {data.reviews.map(r => (
                <div key={r.id} className="p-4 rounded-2xl bg-[#FAF2EC] border border-[#EADBD0] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                  <div>
                    <span className="badge badge-primary">{r.semester}</span>
                    <h4 className="text-sm font-bold text-[#111827] mt-1">{r.reviewName}</h4>
                    <p className="text-slate-500">Date: {r.date} • Time: {r.time} • Venue: {r.venue}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-slate-500">Reviewer:</span>
                    <p className="font-bold text-[#FF5F38]">{r.assignedReviewer}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {data.teams.map(team => (
                  <tr key={team.id}>
                    <td className="py-3 px-3 font-bold text-[#111827]">{team.name} ({team.id})</td>
                    ...
                    <td className="py-3 px-3 text-center font-mono text-[#111827] font-extrabold text-sm">{team.marks?.totalMarks || 48}</td>
                    ...
                    <td className="py-3 px-3 text-right flex justify-end gap-1">
                      <button
                        ...
                        className="px-4 py-2 bg-white hover:bg-slate-100 text-[#0B2E26] font-bold text-xs rounded-xl border border-[#EADBD0] shadow-sm transition-all cursor-pointer py-1 text-[11px]"
                      >
                        Verify 🟢
                      </button>
                      <button
                        onClick={() => updateMarksStatus(team.id, 'Finalized')}
                        className="px-5 py-2.5 bg-[#FF5F38] hover:bg-[#E54D26] text-white font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer py-1 text-[11px]"
                      >
                        Final Lock 🔒
                      </button>
                    </td>
        ...
        <div className="p-6 rounded-3xl border border-[#EADBD0] bg-white shadow-md space-y-4">
          ...
          <div className="space-y-3">
            {data.teams.map(team => {
              ...
              return (
                <div key={team.id} className="p-4 rounded-2xl bg-[#FAF2EC] border border-[#EADBD0] flex items-center justify-between text-xs">
                  <div>
                    <h3 className="font-bold text-[#111827]">{team.name}</h3>
                    <p className="text-slate-500">Mentor: {team.mentorName || 'Unassigned'}</p>
                    <p className="text-[#FF5F38] mt-1 font-semibold">Total Guidance Meetings: {entries.length}</p>
                  </div>
                  <div className="text-right">
